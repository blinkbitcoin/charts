...
class SmokeTest(unittest.TestCase):
    ...
    def test_healthy(self):
        self.assertEqual(self.run_smoke(['{"status":"ok","capabilities":["mint"]}']), (0, 1))

    def test_recovers_from_failed_request(self):
        self.assertEqual(self.run_smoke(['CURL_ERROR', '{"status":"ok","capabilities":["mint"]}']), (0, 2))

    def test_unhealthy(self):
        for body in ['CURL_ERROR', 'not json', '{}', '{"status":"error","capabilities":["mint"]}', '{"status":"ok","capabilities":[]}']:
